...
# ╔═╡ e9ab1f2e-2648-11ee-0c75-91bddbc35926
using SkeelBerzins, LinearSolve, CairoMakie, BenchmarkTools, PlutoUI

# ╔═╡ 2a9d18cb-9395-4e3d-a671-afccf432a62e
md"""
# Benchmark solving PDE using different sparse direct solvers
"""

# ╔═╡ ce62dcd1-73a4-48fa-bd4b-1b6f5fb26eec
...
# ╔═╡ f1179775-3246-443a-91b3-f63b39ea277e
...
# ╔═╡ aa10dc31-6033-42f8-8e51-b956b9396945
function main(Nx, linSolve)
...
# ╔═╡ 806de98b-f06f-423b-895e-03f6fdf5bda3
function scalings(; powmax=9, accurate=false)
	...
	H = [1/n for n in N]
 	...
 	for i=1:length(N)
		...
		if accurate
			t1[i] = @belapsed  main($n, UMFPACKFactorization())
			t2[i] = @belapsed  main($n, KLUFactorization())
			t3[i] = @belapsed  main($n, SparspakFactorization())
		else
			t1[i] = @elapsed  main(n, UMFPACKFactorization())
			t2[i] = @elapsed  main(n, KLUFactorization())
			t3[i] = @elapsed  main(n, SparspakFactorization())
		end
 	end
	...
	f = Makie.Figure()
	ax = Axis(f[1, 1],
			  title = "1D nonlinear diffusion PDE",
			  xlabel = "size N",
    		  ylabel = "time/s",
			  xscale = log10,
			  yscale = log10,
	...
	lines!(ax, N, t1, color=:blue, label="UMFPACK", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, N, t1, color=:blue, markersize = 15)
	lines!(ax, N, t2, color=:red, label="KLU", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, N, t2, color=:red, markersize = 15)
	lines!(ax, N, t3, color=:green, label="Sparspak", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, N, t3, color=:green, markersize = 15)
	
	lines!(ax, N, 9.5e-5*N.^1, color=:orange, label=L"$O(N)$", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ 6e0953d0-87b6-4dda-8a48-14df635204a0
md"""
 __Run Benchmark__: $(@bind allow_run PlutoUI.CheckBox(default=false))
"""

# ╔═╡ dc27f0bd-50e2-4afb-96ab-dbebdadbf52e
if allow_run
	f_test = scalings(powmax=8, accurate=false)
	# save("linearSolve.pdf", f_test, pt_per_unit = 2)
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "068770e8417471808d6a0f3e77bb87314c8f2061"
