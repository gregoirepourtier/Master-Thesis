...
# ╔═╡ 1ba322d0-24bc-11ee-0a36-cbb3a2080255
using SkeelBerzins, SpecialFunctions, LinearAlgebra, LaTeXStrings, PlutoUI, CairoMakie

# ╔═╡ 39d201c9-738e-4613-8395-0405bda5fe79
TableOfContents()

# ╔═╡ d7e04d5d-c66c-4ab6-8c1f-83026f16016d
md"""
# Convergence Results SkeelBerzins.jl
## Stationary Problems
"""

# ╔═╡ f014f0da-9499-438f-9d6b-6a112bd8eae1
# Formulation Problem
begin
	function pdefun_stat(x,t,u,dudx)
		c = 1
		f = 2*u*dudx
		s = -u^2 - 3*exp(2*x)
	...
	function ic_stat(x) # Newton starting value
		u0 = 1
		return u0
	end
	function bdfun_stat(xl,ul,xr,ur,t)
		pl = ul-1.0
		ql = 0
		pr = ur-exp(1.0)
		qr = 0
...
# ╔═╡ 5f9f12f7-4b59-447b-a006-080b1587da74
u_ex1(x) = exp(x) # Exact Solution

# ╔═╡ e5f83def-f257-4df5-808a-3e4e31f03e56
function plot_error_stat(; powmax=11)
	
 	N = (2).^collect(2:powmax)

 	errorL2 = zeros(length(N))
 	errorH1 = zeros(length(N))
	...
	H = [1/n for n in N]
 	...
 	for i=1:length(N)
		...
		sol_pdepe = pdepe(0, pdefun_stat, ic_stat, bdfun_stat, xmesh)
		
		sol_local = sol_ana - sol_pdepe
		
		for j=1:n-1
			errorL2[i] += h*(sol_local[j])^2
			errorH1[i] += h*abs((sol_local[j]+sol_local[j+1])/h)^2
 		end
 		errorL2[i] = sqrt(errorL2[i])
		errorH1[i] = sqrt(errorH1[i])
 	end

	f = Makie.Figure(resolution = (600, 500))
	ax = Axis(f[1, 1],
			  title = L"$L^2$ and $H^1$ errors versus the stepsize $h$",
			  xlabel = L"$h$",
    		  ylabel = "error",
			  xscale = log10,
			  yscale = log10,
	...
	lines!(ax, H, errorL2, color=:blue, label=L"$||u_{exact}-u_{h}||_{L^2}$", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, H, errorL2, color=:blue, markersize = 15)
	lines!(ax, H,  3.5e-2*H.^(2), color=:orange, label=L"$O(h^{2})$", linestyle=:dash,linewidth=3.0)

	lines!(ax, H, errorH1, color=:green, label=L"$||u_{exact}-u_{h}||_{H^1}$", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, H,  errorH1, color=:green, markersize = 15)
	lines!(ax, H, 9.5e-2*H.^(1), color=:red, label=L"$O(h)$", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ c8063324-375a-48ab-b330-185e3bd3b040
md"""
 __Run error Stationary__: $(@bind allow_run1 PlutoUI.CheckBox(default=false))
"""

# ╔═╡ 0372ca9f-9c79-4027-bf4c-e1f344c3e249
if allow_run1
	sol_stat = plot_error_stat( ; powmax=13)
	# save("png/error_stat.pdf", sol_stat, pt_per_unit = 2)
end

# ╔═╡ 72d0ad86-4035-4c1b-8f0c-a488e2ef8e24
md"""
## Instationary Problems
### Nonpolar problem
"""

# ╔═╡ 379b215a-2f0b-4de7-b837-baa4668c1f48
# Formulation Problem
begin
	function pdex1pde(x,t,u,dudx)
		c = pi^2
		f = dudx
		s = 0
...
# ╔═╡ 8c653b1e-c824-474f-b7cd-cbf4e11218b2
u_exact(x,t) = exp(-t)*sin(pi*x) # Exact solution

# ╔═╡ 89815966-d916-4e05-bf06-2c393e3fddd1
function plot_error_nonpolar(; powmax=11)

	points = [11,21,41,81,161]
	tspan = [0.01]
	for i=1:9
		push!(tspan,i/9)
	end

 	error1 = zeros(length(points))
	
	cpt = 1

	H = [1/n for n in points]

 	for i ∈ points
		
		xmesh = collect(range(0,1,length=i))

		params = SkeelBerzins.Params()
		params.solver = :DiffEq
		
		pb        = pdepe(0, pdex1pde, pdex1ic, pdex1bc, xmesh, (0,1) ; params=params)
		problem   = DifferentialEquations.ODEProblem(pb)
		sol_pdepe = DifferentialEquations.solve(problem, Rosenbrock23(), saveat=tspan, abstol=1e-10, reltol=1e-10)

		for j ∈ tspan
			sol_ana = u_exact.(xmesh,j)

			tmp = norm(sol_ana - sol_pdepe(j), Inf)
	...
	f = Makie.Figure(resolution = (600, 500))
	ax = Axis(f[1, 1],
			  title = L"Maximum grid error versus the stepsize $h$",
			  xlabel = L"h",
    		  ylabel = "error",
			  xscale = log10,
			  yscale = log10,
	...
	lines!(ax, H, error1, color=:blue, label=L"max_{t \in [0,1]} \; ||u_{exact}(\cdot,t) - u_h(\cdot,t)||_{\infty}", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, H, error1, color=:blue, markersize = 15)
	lines!(ax, H, 5.5e-1*H.^(2), color=:orange, label=L"$O(h^{2})$", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ f3045773-abc8-480d-89f7-b21eb9b18180
md"""
 __Run error Non Polar Linear__: $(@bind allow_run2 PlutoUI.CheckBox(default=false))
"""

# ╔═╡ 525eb5f9-33da-42e2-bdda-081cc0619f08
if allow_run2
	sol_non_polar = plot_error_nonpolar()
	# save("png/non_polar.pdf", sol_non_polar, pt_per_unit = 2)
end

# ╔═╡ a76cd697-9454-4a4e-80be-f9c9a9e9e419


# ╔═╡ 79a73f0e-4398-4813-8f4e-c0e87e3206b8
...
# ╔═╡ 52d57a09-e0ed-41cc-b995-3fc139cc25b8
# Formulation Problem
begin
...
# ╔═╡ 85e4bd5b-d599-448a-8af3-3a4395928447
function plot_error_nonpolar2(; powmax=11)

	points = [11,22,43,75,161]
	tspan = [0.001]
	for i=1:9
		push!(tspan,i*0.01/9)
	end
	...
	H = [1/n for n in points]
 	...
 	for i ∈ points
		...
		xmesh = collect(range(-1,1,length=i))

		params = SkeelBerzins.Params()
		params.solver = :DiffEq
		
		pb        = pdepe(0, pdefun, icfun, bdfun, xmesh, (0.001,0.01) ; params=params)
		problem   = DifferentialEquations.ODEProblem(pb)
		sol_pdepe = DifferentialEquations.solve(problem, Tsit5(), saveat=tspan, abstol=1e-14, reltol=1e-14)

		for j ∈ tspan
			sol_ana = barenblatt.(xmesh,j,2)
	...
	f = Makie.Figure(resolution = (600, 500))
	ax = Axis(f[1, 1],
			  title = L"Maximum grid error versus the spatial stepsize $h$",
			  xlabel = L"$h$",
    		  ylabel = "error",
			  xscale = log10,
			  yscale = log10,
	...
	lines!(ax, H, error1, color=:blue, label=L"max_{t \in [0.001,0.01]} \; ||u_{exact}(\cdot,t) - u_h(\cdot,t)||_{\infty}", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, H, error1, color=:blue, markersize = 15)
	lines!(ax, H, 8.5e-0*H.^(1), color=:orange, label=L"$O(h)$", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ 151c5d8f-ce7d-4986-a834-b8cb6684f05e
md"""
 __Run error Non Polar Porous__: $(@bind allow_run3 PlutoUI.CheckBox(default=false))
"""

# ╔═╡ c91ba393-b2d4-48f3-a330-babd9c36a3cc
if allow_run3
	sol_non_polar2 = plot_error_nonpolar2()
	# save("png/Porous.pdf", sol_non_polar2, pt_per_unit = 2)
end

# ╔═╡ 99e27800-e2d5-4408-9af3-6f2af8606cda
md"""
### Polar problem
"""

# ╔═╡ ecadc28a-b016-4d30-acb3-074f9a6cd5e1
# Formulation Problem
begin
	function pdefun_cyl(x,t,u,dudx)
	    c = 1
	    f = dudx
	    s = 0
	...
	function icfun_cyl(x)
	    n = 2.404825557695773
	    u0 = besselj(0,n*x)
	    
	    return u0
	end
	function bdfun_cyl(xl,ul,xr,ur,t)
	    n  = 2.404825557695773
...
# ╔═╡ 1f054340-8e1e-4a0f-99b2-504f40d3a407
u_cyl(x,t)=besselj(0,n_0*x)*exp(-n_0^2*t) ; n_0  = 2.404825557695773 # Exact Sol

# ╔═╡ 87a481da-1aa6-44c4-b3e8-2eb7cc81eea1
function plot_error_cyl(; powmax=11)
	...
	H = [1/n for n in points]
 	...
 	for i ∈ points
		...
		pb        = pdepe(1, pdefun_cyl, icfun_cyl, bdfun_cyl, xmesh, (0,1) ; params=params)
		problem   = DifferentialEquations.ODEProblem(pb)
		sol_pdepe = DifferentialEquations.solve(problem, Rosenbrock23(), saveat=tspan, abstol=1e-10, reltol=1e-10)

		for j ∈ tspan
			sol_ana = u_cyl.(xmesh,j)
	...
	lines!(ax, H, error1, color=:blue, label=L"max_{t \in [0,1]} \; ||u_{exact}(\cdot,t) - u_h(\cdot,t)||_{\infty}", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, H, error1, color=:blue, markersize = 15)
	lines!(ax, H, 1.6e-1*H.^(2), color=:orange, label=L"$O(h^{2})$", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ 24dcbe7e-f822-4dd8-b4ba-a44c753b1509
md"""
 __Run error Polar Cylindrical__: $(@bind allow_run4 PlutoUI.CheckBox(default=false))
"""

# ╔═╡ 7ebecfc3-0ed9-4d75-a862-a1e79d199af6
if allow_run4
	sol_cyl = plot_error_cyl()
	# save("png/error_cyl.pdf", sol_cyl, pt_per_unit = 2)
end

# ╔═╡ 342c4306-41af-411a-9bbe-72cb97ead7ee


# ╔═╡ 5608a9c6-8c4d-415c-9613-bbb011970f86
# Formulation Problem
begin
	function pdefun_sph(x,t,u,dudx)
	    c = u
	    f = u*dudx
	    s = 5*u^2 + 4*x*u*dudx
	...
	function icfun_sph(x)
		u0 = exp(1-x^2)
	    
	    return u0
	end
	function bdfun_sph(xl,ul,xr,ur,t)
	    pl = 0 # ignored by solver since m=1
	    ql = 0 # ignored by solver since m=1
	    pr = ur-exp(-t)
	    qr = 0
...
# ╔═╡ bde2fcb9-0128-4f50-8208-54e2b92856b4
u_sph(x,t) = exp(1-x^2-t) # Exact Solution

# ╔═╡ eb2d5c03-ac42-4013-9c49-5683b6afd72a
function plot_error_sph(; powmax=11)
	...
	H = [1/n for n in points]
 	...
 	for i ∈ points
		...
		pb        = pdepe(2, pdefun_sph, icfun_sph, bdfun_sph, xmesh, (0,1) ; params=params)
		problem   = DifferentialEquations.ODEProblem(pb)
		sol_pdepe = DifferentialEquations.solve(problem, Rosenbrock23(), saveat=tspan, abstol=1e-8, reltol=1e-8)

		for j ∈ tspan
			sol_ana = u_sph.(xmesh,j)
	...
	lines!(ax, H, error1, color=:blue, label=L"max_{t \in [0,1]} \; ||u_{exact}(\cdot,t) - u_h(\cdot,t)||_{\infty}", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, H, error1, color=:blue, markersize = 15)
	lines!(ax, H, 7.5e-1*H.^(2), color=:orange, label=L"$O(h^{2})$", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ 91f2e230-1fd7-462e-9ff0-6334f2ba9c31
md"""
 __Run error Polar Spherical__: $(@bind allow_run5 PlutoUI.CheckBox(default=false))
"""

# ╔═╡ b76ffe59-6124-4798-ae29-66cefe5b18b9
if allow_run5
	sol_sph = plot_error_sph()
	# save("png/error_sph.pdf", sol_sph, pt_per_unit = 2)
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "a4647382858af80e5d2cafa487a1b9a251363d69"
