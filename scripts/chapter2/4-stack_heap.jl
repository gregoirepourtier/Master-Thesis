...
# ╔═╡ 988d7576-2be4-11ee-19db-9f17bb10f37a
using SkeelBerzins, CairoMakie, BenchmarkTools, PlutoUI

# ╔═╡ b85fd027-149b-4bf0-bef4-7206187ddf81
md"""
# Benchmark Heap Array vs. Stack Array
"""

# ╔═╡ aea40a3b-6e82-4996-857a-968fdfc87e20
# Formulation Problem with Stack Array
begin
	function pdefun_stack(x,t,u,dudx)
		c = SVector(1,1)
	...
	function bdfun_stack(xl,ul,xr,ur,t)
...
# ╔═╡ 39ab068d-66ba-4a7b-836b-94837705874e
# Formulation Problem with Heap Array
begin
	function pdefun_heap(x,t,u,dudx)
		c = [1,1]
		f = [0.5,0.1] .* dudx
		y = u[1] - u[2]
		s = [-y, y]
	...
	function bdfun_heap(xl,ul,xr,ur,t)
		pl = [ul[1]-1.0, 0]
		ql = [0, 1]
		pr = [0, ur[2]]
		qr = [1, 0]
...
# ╔═╡ 49ebb97a-da70-463b-a447-b357fb1f27e7
function scaling(; powmax=11, accurate=false)
	
	N = (2).^collect(2:powmax)

 	time_stack = zeros(length(N))
 	time_heap  = zeros(length(N))
 	...
 	for i=1:length(N)
		...
		pb_stack = pdepe(0, pdefun_stack, icfun_stack, bdfun_stack, xmesh, (0,10) ; params=params)
		pb_heap  = pdepe(0, pdefun_heap, icfun_heap, bdfun_heap, xmesh, (0,10) ; params=params)

		problem_stack = DifferentialEquations.ODEProblem(pb_stack)
		problem_heap  = DifferentialEquations.ODEProblem(pb_heap)

		if accurate
			time_stack[i] = @belapsed DifferentialEquations.solve($problem_stack, Rosenbrock23())
			time_heap[i]  = @belapsed DifferentialEquations.solve($problem_heap, Rosenbrock23())
		else
			time_stack[i] = @elapsed DifferentialEquations.solve(problem_stack, Rosenbrock23())
			time_heap[i]  = @elapsed DifferentialEquations.solve(problem_heap, Rosenbrock23())
		end
 	end

	f = Makie.Figure(resolution = (600, 500))
	ax = Axis(f[1, 1],
			  title = "Timings for Static Arrays vs. Arrays",
			  titlefont = :regular,
			  xlabel = "size N",
    		  ylabel = "time/s",
			  xscale = log10,
	...
	lines!(ax, N, time_stack, color=:blue, label="stack", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, N, time_stack, color=:blue, markersize = 15)

	lines!(ax, N, time_heap, color=:green, label="heap", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, N,  time_heap, color=:green, markersize = 15)
...
# ╔═╡ c49179b9-277c-4224-8183-0508d69a5e7a
md"""
 __Run Benchmark__: $(@bind allow_run PlutoUI.CheckBox(default=false))
"""

# ╔═╡ 22352094-202e-4018-9282-d8867c5a6027
if allow_run
	res_scaling = scaling(powmax=13, accurate=false)
	# save("scaling.pdf", res_scaling, pt_per_unit = 2)
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "9a499ae84a0d6cd9c9a0f0746d590675323341fa"
