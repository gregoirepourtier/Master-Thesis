...
# ╔═╡ 6aa16ffc-3062-11ee-34dc-5785f52e0578
using DifferentialEquations, CairoMakie, Colors

# ╔═╡ bdbaa0a7-5269-4800-a359-fe61cfcea7f1
md"""
# Stiff ODE
"""

# ╔═╡ 247e0e83-2ce9-42ad-87a0-2ada6086f9ee
function assemble(du,u,p,t)
	du[1] = -10*u[1]
	du[2] = -0.1*u[2]
end

# ╔═╡ ad467af6-77d5-4651-ba40-73095ab6173e
begin
	f(u, p, t) = -15 * u
	u0 = [1.,1.]
	tspan = (0.0, 10.0)
	prob = ODEProblem(assemble, u0, tspan)
	sol = solve(prob, Euler() ; dt=0.194,adaptive=false) # dt, dtmin=dt, dtmax=dt, force_dtmin=true)
end ;

# ╔═╡ 372da15a-89f3-4991-a179-deb1ec3749b0
function plot_res()
	f  = Makie.Figure(resolution = (800, 500))
	ax = Axis(f[1, 1],
			  xlabel = "time t",
    		  ylabel = "solution u",
			  xticks = 0:2:10)
	
	lines!(ax, sol.t, sol[1,:], color=:blue, label=L"$u_1$ Euler", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, sol.t, sol[1,:], color=:blue, markersize = 10,marker=:xcross)
	lines!(ax, sol.t, sol[2,:], color=:red, label=L"$u_2$ Euler", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, sol.t, sol[2,:], color=:red, markersize = 10,marker=:xcross)

	ylims!(-1.05,1.05)

	lines!(ax, sol.t, t -> exp(-10t), color=:orange, label=L"$u_1$ exact", linestyle=:dash,linewidth=3.0)
	lines!(ax, sol.t, t -> exp(-0.1t), color=:yellow, label=L"$u_2$ exact", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ 39b8c97a-d9ea-49db-83e6-0bd5585df629
res_plot = plot_res()

# ╔═╡ 3040a3de-3c1d-4e5f-933c-412316d58244
# save("figures/stiffODE.pdf", res_plot, pt_per_unit = 2)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "f842f19b692fe75922e5d99d52e3d232ca4f915c"
