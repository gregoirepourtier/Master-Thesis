...
# ╔═╡ 7758984c-2d3b-11ee-3db0-d5e91734d776
using SkeelBerzins, CairoMakie, LinearAlgebra, PlutoUI

# ╔═╡ ed01c6cd-8d71-4896-9ed7-3fdccd6c2a38
md"""
# Convergence Results for time integration techniques
"""

# ╔═╡ ea7b2c2f-2f5a-4d63-b6e5-e94e645e51df
# Formulation Problem
begin
...
# ╔═╡ 23367f02-e8dc-4efd-8442-4767dae64c58
exact_test(x,t) = exp(-t)*sin(pi*x) ; # Exact Solution

# ╔═╡ 5131c129-8cb0-4bd3-898b-a0043b820ef5
function main_DiffEq(Nx)
	...
	pb = pdepe(0, pdex1pde, pdex1ic, pdex1bc, xmesh, tspan ; params=params)

	xmesh, pb
end

# ╔═╡ 75342819-f371-47c6-a0d2-bcd5cecad16e
function main_Euler(Nx,dt)
	...
	sol = pdepe(0, pdex1pde, pdex1ic, pdex1bc, xmesh, tspan ; params=params)

	xmesh, sol
end

# ╔═╡ 3096ce7b-4fca-4b7a-971a-bcc8428f35c5
dts(n)=[0.1*2.0^(-i) for i=1:n]

# ╔═╡ ead6fb36-2512-48fd-881d-c43150e0691d
function errortest(mesh,pb,dt,method,idx_x)
	
	problem = DifferentialEquations.ODEProblem(pb)

	sol = DifferentialEquations.solve(problem, method; dt, dtmin=dt, dtmax=dt, force_dtmin=true, abstol=1.0, reltol=1.0 , maxiters=1.0e5)

	error = 1
	for idx ∈ idx_x
		sol_ana = exact_test.(mesh[idx],sol.t)

		tmp = norm(sol_ana - sol[idx,:], Inf)
...
# ╔═╡ a518b508-fc64-4e7c-b88c-a256ac9e03a5
function error_time(n_step)
	
	dt      = dts(n_step)
	Nx_low  = 10_000 .* ones(Int,n_step)
	Nx_high = [2000, 7500, 20_000, 30_000]
	...
	for i in eachindex(dt)
		...
		mesh,  pb  = main_DiffEq(Nx1)
		mesh1, pb1 = main_DiffEq(Nx2)
		...
		t = dt[i]
		mesh2, sol = main_Euler(Nx1,t)
		error = 1
		for idx ∈ idx_x1
			sol_ana = exact_test.(mesh2[idx],sol.t)
		...
		error1[i] = errortest(mesh,pb,t,Rosenbrock23(),idx_x1)
		error2[i] = errortest(mesh,pb,t,QNDF2(),idx_x1)
		error3[i] = errortest(mesh1,pb1,t,Rodas5(),idx_x2)
		# @info error3[i]
	end
	...
	lines!(ax, dt, error4, color=:pink, label="ImplicitEuler", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, dt, error4, color=:pink, markersize = 15)

	lines!(ax, dt, error2, color=:red, label="QNDF2", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, dt, error2, color=:red, markersize = 15)

	lines!(ax, dt, error1, color=:blue, label="Rosenbrock23", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, dt, error1, color=:blue, markersize = 15)
	
	lines!(ax, dt, error3, color=:green, label="Rodas5", linestyle=:solid,linewidth=2.0)
	Makie.scatter!(ax, dt, error3, color=:green, markersize = 15)


	lines!(ax, dt, 1.5e-1*dt.^(1), color=:black, label=L"$O(h)$", linestyle=:dash,linewidth=3.0)
	
	lines!(ax, dt, 2.5e-2*dt.^(2), color=:orange, label=L"$O(h^{2})$", linestyle=:dash,linewidth=3.0)

	lines!(ax, dt, 7.5e-2*dt.^(5), color=:brown, label=L"$O(h^5)$", linestyle=:dash,linewidth=3.0)
...
# ╔═╡ c779a694-d523-4f58-ba29-9603a4bb2eb8
md"""
 __Run convergence tests__: $(@bind allow_run PlutoUI.CheckBox(default=false))
"""

# ╔═╡ 9ff45cf8-0f89-4111-8e49-fedf9d6e8f41
if allow_run
	plot_res = error_time(4)
	# save("scaling_time.pdf", plot_res, pt_per_unit = 2)
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "9cce995733a5db3cab91d35f97d850be223e9317"
